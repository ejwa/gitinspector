...
(function () {
	...
	var THEMES = {"default": "default", terminal: "terminal"};
	var MODES = {light: "light", dark: "dark"};
	var MINORS = {shown: "shown", hidden: "hidden", "true": "shown", "false": "hidden", yes: "shown", no: "hidden"};

	/* A link can carry ?theme=terminal&mode=dark&minor=shown to open the report in a given view. The
	   parameter only steers this visit and is never stored, so it cannot outlive the link and turn
	   into the choice the reader made themselves. */
	function parameter(name, values) {
		var match = new RegExp("[?&]" + name + "=([^&]*)").exec(window.location.search);
		var value = match ? decodeURIComponent(match[1].replace(/\+/g, " ")).toLowerCase() : "";

		return values.hasOwnProperty(value) ? values[value] : null;
	}
	...
	function preferredMode() {
		var chosen = parameter("mode", MODES) || recall(STORE_MODE);
	...
	function ready() {
		apply(parameter("theme", THEMES) || recall(STORE_THEME) || "default", preferredMode());
		...
		applyMinor((parameter("minor", MINORS) || recall(STORE_MINOR)) === "shown");
